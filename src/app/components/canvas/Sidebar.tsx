...
function Sidebar({width, height}: SidebarPorps) {
        ...
        Object.keys(nodeState).forEach((v, i) => {
            ...
            if (!nodeState[v].active) {
                nodeDispatch({
                    type: 'updateNode',
                    nodeID: v,
                    value: {x: x, staticX: x, sideBarX: x}
                })
            }
        })
    ...
    const category = (name: string, x: number, y: number, fill: string) => (
        <>
    ...
    return (
        ...
        >
            ...
            <Line
                ...
                y={0}
            />
            {category('Common Nodes', 0.99, 35, '#d0f4de')}
            {nodeState &&
            ...
            {category('Machine learning', 0.99, sideBar.width > 300 ? 315 : 310, '#e4c1f9' )}
            {nodeState &&
                Object.keys(nodeState).map(v => (
                    <>
