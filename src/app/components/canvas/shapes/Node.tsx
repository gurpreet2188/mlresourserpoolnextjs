...
import AnchorBase from './AnchorBase'
import {nodeConfig} from "@/app/nodes/config";
...
function Node({id}: NodeProps) {
    ...
    const [nodeSeleted, setNodeSeteled] = useState(false)
    ...
    useEffect(()=>{
        if (!nodeState[id].active){
            setNodeSeteled(false)
        }
    },[nodeState[id].active])
    const clickHandle = () => {
        setNodeSeteled(!nodeSeleted)
